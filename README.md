...
- #### Upper bounds
  ...
  >
  > The type specified after a colon is the **upper bound**, indicating that only a subtype of `Comparable<T>` can be
  substituted for `T`.
  >
...
```java
public class
RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2, NestedScrollingChild3 {
}
```
...
```java
public class
ConcatAdapter extends RecyclerView.Adapter {
}
```
...
- [Jetpack Library Releases: RecyclerView](https://developer.android.com/jetpack/androidx/releases/recyclerview)
- [Android developers: ConcatAdapter](https://developer.android.com/reference/androidx/recyclerview/widget/ConcatAdapter)
- [Medium: Concatenate adapters sequentially with ConcatAdapter](https://medium.com/androiddevelopers/merge-adapters-sequentially-with-mergeadapter-294d2942127a)
- [Beginner's Guide on ConcatAdapter in Android](https://wajahatkarim.com/2022/06/concat-adapter/)
...
- [Medium: Understanding @Stable, @Immutable, ImmutableCollection, and PersistentCollection in Jetpack Compose](https://blog.kotlin-academy.com/understanding-stable-immutable-immutablecollection-and-persistentcollection-in-jetpack-compose-5b352f3a265f)
...
- 👉 GC — это механизм сбора неиспользуемых объектов и освобождения памяти для дальнейшего использования
- 👉 До сих пор существовало 5 версий GC в Android, а именно:

  - 📌 Dalvik GC (до KitKat): Stop the World GC
  - 📌 ART GC (Lollipop и Marshmallow): Generational GC
  - 📌 ART GC (Nougat): ART/Dalvik Android Team переписывает весь процесс распределения в ассемблере
  - 📌 ART GC (Oreo): Параллельное копирование GC
  - 📌 ART GC (Android 10 и более поздние версии): повторно представлен Generational GC как расширение Concurrent Copying GC

#### 📌 Dalvik GC (до KitKat): Stop the World GC (https://miro.medium.com/max/1400/1*ibInI6p9t5rpDawUc-yHIA.gif)

- 👉 Этот сборщик мусора работает на алгоритме одновременной маркировки и очистки
- 👉 Давайте разберемся с этим алгоритмом:

1️⃣ Поиск корней
- 👉 Сборщик мусора приостанавливает все потоки в системе, чтобы найти корни
- 👉 Эта фаза не параллельна, поэтому определение корней требует времени, и в это время приложение ничего не может сделать
- 👉 На этом этапе логический флаг marked по умолчанию устанавливается в false для каждого объекта

2️⃣ Отметить доступные объекты
- 👉 Объекты, имеющие прямое или косвенное отношение к корням, на этом этапе помечаются как доступные
- 👉 Эта фаза является параллельной
- 👉 На этом этапе флаг marked устанавливается в true для каждого объекта, который является достижимым
- 👉 На этом этапе недоступные объекты не собираются сразу. Это откладывается до тех пор, пока вся доступная память не
будет исчерпана

- 👉 Выполняется обход с поиском в глубину

3️⃣ Очистка недоступных объектов
- 👉 Объекты, не помеченные как достижимые на предыдущем этапе, считаются недостижимыми и очищаются из кучи

➖
- 👉 Короткоживущие объекты остаются в памяти намного дольше после того, как становятся неиспользуемыми
- 👉 Приостанавливается поток приложения для его работы
- 👉 Происходит фрагментация, которая может привести к ошибке OutOfMemory

#### 📌 Параллельное копирование GC

- 👉 Переписали весь коллектор. Эта новая версия поставляется со стратегией постоянной дефрагментации для решения проблемы
уплотнения кучи
- 👉 Они сделали GC сжатием кучи всегда необходимым, даже на переднем плане. Из-за этого GC может чаще сжимать кучу
приложений, которые всегда находятся на переднем плане, а также приложения в фоновом режиме
- 👉 Этот процесс является параллельным
- 👉 Сборщик мусора разделил кучу на сегменты. Когда потоку требуется память, сборщик мусора предоставляет этому потоку
корзину для локального выделения объектов вместо блокировки всей системы
- 👉 Когда корзина дефрагментирована, если она используется менее 70% или 75%, эта корзина будет собрана, а находящиеся в
ней объекты будут перемещены в другую корзину, опустошая первую
- 👉 Они также представили: метод bump-the-pointer для выделения объектов
- 👉 При использовании этого метода указателей единственное, что необходимо для размещения нового объекта, — это
переместить указатель в определенную корзину
- 👉 Когда потоку передается корзина, он проверяет, помещается ли новый объект в оставшееся пространство между указателем и
концом корзины
- 👉 Если да, выделяется новый объект, и указатель обновляется адресом нового объекта
- 👉 Этот метод позволяет в 18 раз быстрее выделить время, чем Dalvik, и на 70% быстрее, чем Nougat

➖
- 👉 Удаление Generational GC
...
- [Android Garbage Collection in a Nutshell](https://medium.com/@banerjee.s.sayans/android-garbage-collection-in-a-nutshell-e5c8acfa1538)
