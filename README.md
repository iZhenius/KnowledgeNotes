...
👉 На этом этапе недоступные объекты не собираются сразу. Это откладывается до тех пор, пока вся доступная память не будет исчерпана
...
👉 Переписали весь коллектор. Эта новая версия поставляется со стратегией постоянной дефрагментации для решения проблемы уплотнения кучи
👉 Они сделали GC сжатием кучи всегда необходимым, даже на переднем плане. Из-за этого GC может чаще сжимать кучу приложений, которые всегда находятся на переднем плане, а также приложения в фоновом режиме
👉 Этот процесс является параллельным
👉 Сборщик мусора разделил кучу на сегменты. Когда потоку требуется память, сборщик мусора предоставляет этому потоку корзину для локального выделения объектов вместо блокировки всей системы
👉 Когда корзина дефрагментирована, если она используется менее 70% или 75%, эта корзина будет собрана, а находящиеся в ней объекты будут перемещены в другую корзину, опустошая первую
👉 Они также представили: метод bump-the-pointer для выделения объектов
👉 При использовании этого метода указателей единственное, что необходимо для размещения нового объекта, — это переместить указатель в определенную корзину
👉 Когда потоку передается корзина, он проверяет, помещается ли новый объект в оставшееся пространство между указателем и концом корзины
...
Далее автор рассказывает про Generational GC, что произойдет, если что-то пойдет не так и на картинке показывает как GC определяет какой объект подходит под чистку. Получается там еще много чего интересного, так что кто хочет чуть погрузиться в мир GC welcome 👍
